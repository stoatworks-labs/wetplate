...
- The browser demo's shaders are the plugin's, character for character: `python3 demo/tools/check_shaders.py`
  (in verify.sh). The demo's CPU half (`demo/plugin.js`: the clock, the ring, the window, the take,
  the laws) is a hand port; only a reader checks it. Its sum pass is the page's own, because the
  plugin's will not compile in GLSL ES 3.00 — see AGENTS.md, *The browser demo*.
- Deploy the demo: `cf-run npx wrangler deploy` from the repo root (a push to main also deploys it);
  verify by content: `curl -s 'https://wetplate-demo.stoatworks-labs.com/?cb=1' | grep -o '<title>[^<]*'`
...
- **Never loaded into Resolume on macOS.** Everything numeric is measured offline on
  macOS against the real plugin class in a headless CGL context, plus an `oxbow` load.
- On Windows the v0.1.0 DLL ran in Arena 7.27.1 on win-lab (software rendering) through
  the fleet gate: 8 of 9, Exposure and Buckets dead on the still carrier; the verdict is
  in AGENTS.md and the README.
- Seen on the synthetic card and on Resolume's bundled demo clips through `--pipe` (the
  project video); never on camera footage of people or places.
- No OpenFX port, no factory presets, no seed control. The browser demo's CPU half is a
  port nothing checks but a reader (`demo/`); its sum pass is the page's own.
